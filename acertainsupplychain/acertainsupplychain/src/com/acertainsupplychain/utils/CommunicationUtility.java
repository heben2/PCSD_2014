...
import com.acertainsupplychain.proxy.ProxyConstants;
...
public final class CommunicationUtility {
    ...
    /**
     * Convert a request URI to the message tags supported in acertainsupplychain
     * 
     * @param requestURI
     * @return
     */
     ...
     * response and unpacks the response. For lists.
     * 
     * @param client
     * @param exchange
     * @return A List<?> for a get function, otherwise null
     * @throws CommunicationException
     */
    public static List<?> SendAndRecv(HttpClient client,
            ContentExchange exchange) throws OrderProcessingException, Exception {
        int exchangeState;
        try {
            client.send(exchange);
        } catch (IOException ex) {
            throw new Exception(
                    ProxyConstants.strERR_CLIENT_REQUEST_SENDING, ex);
        }
        ...
        if (exchangeState == HttpExchange.STATUS_COMPLETED) {
            try {
                CommunicationResponse CommunicationResponse = (CommunicationResponse) CommunicationUtility
                        .deserializeXMLStringToObject(exchange
                                .getResponseContent().trim());
                Exception ex = CommunicationResponse.getException();
                if (ex != null) {
                    throw new OrderProcessingException(ex);
                }
                return CommunicationResponse.getList();
                
            } catch (UnsupportedEncodingException ex) {
                throw new Exception(
                        ProxyConstants.strERR_CLIENT_RESPONSE_DECODING,
                        ex);
            }
        } else if (exchangeState == HttpExchange.STATUS_EXCEPTED) {
            throw new Exception(
                    ProxyConstants.strERR_CLIENT_REQUEST_EXCEPTION);
        } else if (exchangeState == HttpExchange.STATUS_EXPIRED) {
            throw new Exception(
                    ProxyConstants.strERR_CLIENT_REQUEST_TIMEOUT);
        } else {
            throw new Exception(
                    ProxyConstants.strERR_CLIENT_UNKNOWN);
        }
    }
    
    /**
     * Manages the sending of an exchange through the client, waits for the
     * response and unpacks the response. For ids (int).
     * 
     * @param client
     * @param exchange
     * @return id (int) for the register function, otherwise null
     * @throws CommunicationException
     */
    public static int SendAndRecvId(HttpClient client,
        ...
        if (exchangeState == HttpExchange.STATUS_COMPLETED) {
            try {
                ...
                return CommunicationResponse.getId();
